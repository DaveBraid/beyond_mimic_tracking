...
class RewardsCfg:
    ...
    此处正则表达式工作机制
        开头 ^：强制从关节名称的开头开始检查
        多个负向前瞻 (?!...)：检查关节名称是否不等于指定的名称
        left_foot_roll_link、right_foot_roll_link：脚部关节
        left_hand_link、right_hand_link：手部关节
        left_knee_link、right_knee_link：膝盖关节
        结尾 .+$：确保匹配至少一个字符后的任意内容（确保名称不为空）

        这意味着："匹配所有关节名称，但排除脚、手和膝盖关节"。也就是说，它会选中基座、肩膀、肘部等其他关节进行接触检测，而脚、手、膝盖的接触会被忽略。
    """
...
class TerminationsCfg:
    ...
    ee_body_pos = DoneTerm(  # End Effector body position termination 末端执行器身体位置终止
...
@configclass
class TrackingEnvCfgS3(ManagerBasedRLEnvCfg):
